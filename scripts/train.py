...
# sys.path.append('/content/src/consistency-models')
import consistency_models
...
DATA_DIR = "consistency_models_2/data"
...
# Instantiate the data module with the default configuration
...
def save_images_grid(images):
    # Unnormalize the images for display
    images = images / 2 + 0.5  # Reverse the normalization applied during transforms
    np_images = images.numpy()
    # plt.figure(figsize=(10, 10))
    fig, ax = plt.subplots(figsize=(8, 4))
    plt.axis('off')
    # Adjust nrow as per batch size or preference
    plt.imshow(np.transpose(make_grid(images, nrow=8, padding=0), (1, 2, 0)))

    # save the figure
    data_dir = os.path.join(DATA_DIR, 'samples',
                            ImageDataModuleConfig.data_dir)

    # make the directory and its parents if they don't exist
    os.makedirs(data_dir, exist_ok=True)

    # Remove padding and margins from the figure and all subplots
    ...
    plt.savefig(os.path.join(
        data_dir, 'images_org.png'))


# Fetch and plot the shape of a few samples
for images, _ in train_loader:
    # Print the shape of the images tensor
    print("Batch Images shape:", images.shape)
    save_images_grid(images)
...
class NoiseLevelEmbedding(nn.Module):
    def __init__(self, channels: int, scale: float = 0.02) -> None:
        ...
        self.W = nn.Parameter(torch.randn(channels // 2)
...
class UNet(nn.Module):
    def __init__(self, config: UNetConfig) -> None:
        ...
        self.top_encoder_blocks = self._make_encoder_blocks(
            self.config.top_blocks_channels +
            self.config.mid_blocks_channels[:1],
        ...
        self.mid_encoder_blocks = self._make_encoder_blocks(
            self.config.mid_blocks_channels +
            self.config.mid_blocks_channels[-1:],
    ...
    def _make_encoder_blocks(
        ...
        for idx, (in_channels, out_channels) in enumerate(channel_pairs):
            for _ in range(n_blocks_per_resolution[idx]):
                blocks.append(
                    block_fn(in_channels, out_channels, dropout[idx]))
...
unet_summary = summary(UNet(UNetConfig()), input_size=((1, 3, 32, 32), (1,)))
# print(unet_summary)
...
class LitImprovedConsistencyModel(LightningModule):
    ...
    def training_step(self, batch: Union[Tensor, List[Tensor]], batch_idx: int) -> None:
        ...
        loss = (
            pseudo_huber_loss(output.predicted, output.target) *
            output.loss_weights
        ).mean()

        self.log_dict(
            {"train-loss": loss, "num-timesteps": output.num_timesteps})
    ...
    def on_train_batch_end(
        ...
        update_ema_model_(self.model, self.ema_model,
                          self.config.ema_decay_rate)
    ...
    @torch.no_grad()
    def __log_images(self, images: Tensor, title: str, step: int) -> None:
        ...
        self.logger.log_image(key=title, images=[grid], step=step)
...
class TrainingConfig:
    ...
    trainer: Trainer
    seed: int = 42
    model_ckpt_path: str = os.path.join(DATA_DIR, "checkpoints/icm")
...
def run_training(config: TrainingConfig) -> None:
    ...
    # Finish logging to avoid broken pipe error
    wandb.finish()
...
training_config = TrainingConfig(
    ...
    trainer=Trainer(
        ...
        logger=WandbLogger(project="consistency-models",
